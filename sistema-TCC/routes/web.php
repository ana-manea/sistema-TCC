...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/aluno/entregas', [EntregaController::class, 'indexOrientando'])->name('aluno.entregas.index');

    // ── Arquivos de Entrega ────────────────────────────────────────────────────
    Route::resource('arquivos_entrega', ArquivoEntregaController::class); // apenas uma vez

    // ── Bancas e Avaliações ────────────────────────────────────────────────────
    Route::resource('bancas', BancaController::class);
    Route::post('/bancas/{banca}/confirmar-realizada', [BancaController::class, 'confirmarRealizada'])->name('bancas.confirmarRealizada');
...
Route::get('/aluno/tarefas', [TarefaController::class, 'indexOrientando'])->name('aluno.tarefas.index');
...
Route::controller(OrientadorController::class)->group(function () {
    Route::get('/orientador/{orientador}/meus-orientandos', 'meusOrientandos')->name('orientador.meus_orientandos');
});
...
Route::controller(SolicitacaoOrientadorController::class)->group(function () {
    Route::put('/solicitacoes_orientador/{solicitacaoOrientador}/responder', 'responder')->name('solicitacoes_orientador.responder');
    Route::get('/solicitacoes_orientando', 'indexOrientando')->name('solicitacoes_orientando.index');
    Route::get('/solicitacoes_orientando/create', 'createOrientando')->name('solicitacoes_orientando.create');
    Route::get('/orientador/{orientador}/solicitacoes', 'index')->name('solicitacoes_orientador.index');
    Route::post('/solicitacoes_orientador', 'store')->name('solicitacoes_orientador.store');
});
